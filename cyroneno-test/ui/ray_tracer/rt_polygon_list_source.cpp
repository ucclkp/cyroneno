...
namespace cyro {
        ...
        ukive::ListItem* item)
    {
        ...
        auto b = polygon_item->item_view->getBackground();
        if (data.selected) {
            if (b) {
                static_cast<ukive::ColorElement*>(b)->setColor(ukive::Color::Blue200);
            } else {
                polygon_item->item_view->setBackground(new ukive::ColorElement(ukive::Color::Blue200));
            }
        } else {
            if (b) {
    ...
    void RTPolygonListSource::selectItem(int sel_pos) {
        if (sel_pos < -1 ||
